        ...
        )
{
    ...
    return std::make_tuple(weight_hidden_output, weight_input_hidden, err);
}

std::vector< long double > eval(
        std::vector<std::vector<long double> >& train_test,
...
int main() {
    ...
    volatile const auto reset = false;
    const auto hlr = 0.4l;
    ...
    const auto tr_res = training(hidden_neurons, hlr, epochs, train_out, train_inp);
